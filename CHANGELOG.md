...
### 新增 (Added)
- **文字 (Label) 系统**：全新的文字管理模块 (`cf.label`)。
  - **链式 API**：支持设置文本、字体、颜色、背景、偏移、缩放、可见性等。
  - **调试面板**：独立的 Label 调试面板，支持实时属性调整、高度控制及配置代码一键复制。
  - **深度检测控制**：新增 `setDisableDepthTestDistance`，默认开启“始终置顶”模式，解决文字被地形或模型遮挡的问题。
  - **高度控制**：`setHeight` 支持智能切换高度模式（贴地/相对），与 Point 逻辑保持一致。
- **调试器 (Debugger) 增强**：
  - **SVG 引导线**：新增连接调试面板与实体的 SVG 引导线，支持地形遮挡检测，精准指向物体屏幕位置。
  - **体验优化**：统一了各类型面板的 UI 风格（颜色选择器、输入框宽度等）。
  - **配置生成**：调试面板支持生成两种格式的配置代码（对象字面量 & 链式调用）。

### 修复 (Fixed)
- 修复广告牌 (Billboard) 调试时缺少引导连接线的问题。
- 修复调试器引导线在贴地模式下指向地下的问题（现已适配地形高度）。
- 修复 Label `setHeight` 在实体初始化前调用失效的问题。
- 移除 Label 冗余的 `setHeightOffset` 方法，规范化 API。
- 修复 Point 内部调用 Label 旧 API 导致的兼容性问题。
